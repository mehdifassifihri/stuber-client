...
const handleSubmit = (event) => {
  ...
  axios.post('http://localhost:8080/Bus', data)
    .then(response => {
...
const handleDelete = (id) => {
  axios.delete(`http://localhost:8080/Bus/${id}`)
    .then((response) => {
  ...
  <div className='text-black ml-2'><span className='font-bold'>Buses Management</span></div>
  </div>
  <input placeholder='Search' className='bg-zinc-100 rounded-3xl placeholder:px-3 focus:outline-none focus:placeholder-transparent text-black text-xs px-4 py-3' type="text" 
         onChange={(e) => handleSearch(e.target.value)}
         value={searchTerm}/>
  </div>
  <form className='flex justify-center gap-2 mt-4'>
  <input placeholder='Reference' onChange={handleReferenceChange} className='bg-zinc-100 rounded-3xl placeholder:px-3 focus:outline-none focus:placeholder-transparent text-black' type="text"/>
  ...
  <Modal  title="Edit Driver" open={isModalOpen} onOk={handleOk} onCancel={handleCancel}>
        <form >
